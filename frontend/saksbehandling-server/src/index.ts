...
const ekspertbistandBackendProxy = createProxyMiddleware({
  ...
  on: {
    ...
    error(err: Error, req: IncomingMessage, res: ServerResponse | Socket) {
      logger.error({ err, path: req.url }, "Proxy error mot saksbehandling-api");
...
const resolveStaticDir = (): string => {
  const defaultDir = path.resolve(__dirname, "../../saksbehandling/dist");
  const resolvedPath = STATIC_DIR ? path.resolve(__dirname, STATIC_DIR) : defaultDir;
...
app.listen(port, () => {
  logger.info(`🚀 Ekspertbistand saksbehandling BFF started on ${port} with basepath ${basePath}`);
});
